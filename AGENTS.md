...
Editing or reviewing these skill documents does not activate the workflow they describe. Treat skill maintenance as a direct documentation task unless the user explicitly asks to run the multi-agent chain. Mentions of roles, models, agents, tickets, or retries are document content, not delegation instructions. For every task, identify the requested deliverable and choose the smallest sufficient process. Invoking one skill does not automatically activate the entire chain. Keep the execution method proportional to the requested deliverable. Do not create supporting specs, tickets, handoffs, review agents, or retry records for a simple documentation edit.
