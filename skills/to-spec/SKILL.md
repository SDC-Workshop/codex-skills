...
Hand `/to-tickets` the complete spec revision, requirement IDs and evidence, work packages, contracts, dependency implications, scope exclusions, open decisions, and acceptance boundary. It must turn each package into a one-outcome ticket that fits one fresh ORCHESTRATOR implementation context, including worker handoff and review. Do not close or modify a parent issue as part of spec creation or routine planning. ARCHITECT may revise the parent spec and affected dependent tickets during an authorized return cycle.

### 5. Define completion

- CODING completion means the assigned implementation change and evidence are submitted within the ticket boundary. CODING does not close the ticket.
- ORCHESTRATOR completion means independent checks of accuracy, documented standards and compliance, completeness, integration, and every required acceptance result. A worker message or green tests alone is insufficient. ORCHESTRATOR records the candidate/base evidence, verdict, attempts, and next action.
- ARCHITECT completion means the recorded ORCHESTRATOR evidence satisfies this spec and its contracts. ARCHITECT may correct the spec or affected tickets and must not take over implementation coding or start an open-ended duplicate audit.

If an ARCHITECT revision changes a requirement or dependency, put affected downstream tickets on hold, reconcile their contracts and readiness, and invalidate stale affected acceptance evidence while preserving evidence that remains valid.
